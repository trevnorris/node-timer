...
initAsync.prototype = {
  ...
  onend: function onend(fn, usr_args) {
    if (usr_args)
      this._args_end = usr_args;
    return setOn(this, '_usr_end', fn);
  },
  oninterval: function(fn, usr_args) {
    if (usr_args)
      this._args_interval = usr_args;
    return setOn(this, '_usr_interval', fn);
  },
  onstart: function(fn, usr_args) {
    if (usr_args)
      this._args_start = usr_args;
    return setOn(this, '_usr_start', fn);
  }
};

function setOn(ts, name, fn) {
  if (typeof fn != 'function')
    throw TypeError('callback must be a function');
  ts[name] = fn;
  return ts;
...
function asyncIntStart() {
  ...
  _this._setinterval = setInterval(function asyncSetInterval() {
    ...
    _this._interval(_this.name,
                    prev_time,
                    _this._counter,
                    _this._args_interval);
