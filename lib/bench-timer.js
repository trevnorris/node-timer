 ...
 *  }
 *
 * There is one flag reserved by timer. The `--exec` flag indicates which timers
 * the user wants to run. If no parameters are passed to `--exec`, then no tests
 * will run.
 *
...
function timer(name, fn, term, kill) {
  // --exec causes problems with async tests, so don't filter those for now
  if (to_exec && !to_exec[name] && typeof fn == 'function')
    return;
...
timer.parse = function(argv) {
  ...
  for (i = 2; i < argv.length; i++) {
    if (rx_parse_params.test(argv[i])) {
      ...
      tmpargv = isFinite(argv[i]) ? parseInt(argv[i]) : argv[i];
      if (obj[current] === true) {
        obj[current] = tmpargv;
      } else {
